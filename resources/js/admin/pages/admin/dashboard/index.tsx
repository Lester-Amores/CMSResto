...
import { TrendingUp, ShoppingCart, CalendarDays, ClipboardList, Building2 } from 'lucide-react';
import { StatBlock } from './stat-block';
import { fetchAdminSalesData } from '@/admin/services/services';
...
export default function Dashboard({ data }: Props) {
    ...
    const { data: salesData } = useQuery({
        queryKey: ['fetchAdminSalesData', range, date],
        queryFn: () => fetchAdminSalesData({ range, date }),
    });

    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="Dashboard" />
            <div className="flex h-full flex-1 flex-col gap-6 p-6 sm:min-w-[calc(100%-16rem)]">
                <div className="grid grid-cols-1 sm:grid-cols-1 lg:grid-cols-5 gap-4">
                    ...
                    />
                    <StatBlock
                        icon={<Building2 className="w-6 h-6 text-stone-600 dark:text-stone-200" />}
                        label="Total Branches"
                        value={data?.branchCount?.toString()}
                        bg="bg-stone-50 dark:bg-stone-700"
                        />
                </div>
