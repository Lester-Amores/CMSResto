...
import type { BreadcrumbItem, PaginatedProps, Branch, FlashMessages, ModalMode } from '@/admin/types';
...
export default function BranchPage({ branches, current_page, total_pages, total_rows, per_page }: BranchProps) {
    ...
    const handleRestore = (id: number) => {
        ...
        showConfirmationPopup({
            ...
            cancelButton,
            onSuccess: () => {
                router.post(route('branches.restore'), { id }, {
                    preserveScroll: true,
                    onSuccess: (page) => {
                        const flash = page.props.flash as FlashMessages;
                        handleFlashMessages(flash)
    ...
    const handleMultiDelete = () => {
        ...
        showConfirmationPopup({
            ...
            cancelButton,
            onSuccess: () => {
                router.post(route('branches.multi-delete'), { ids: selectedIdsRef.current }, {
                    headers: {
    ...
    const handleMultiRestore = () => {
        ...
        showConfirmationPopup({
            ...
            cancelButton,
            onSuccess: () => {
                router.post(route('branches.multi-restore'), { ids: selectedIdsRef.current }, {
                    headers: {
                        'Content-Type': 'application/json',
                    },
                    preserveScroll: true,
    ...
    const data = branches?.map(branch => {
        const isDeleted: boolean = !!branch.deleted_at;
        return {
            id: branch.id,
            name: branch?.name,
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <SlidingModal isOpen={isModalOpen} onClose={closeModal} size={modalMode === 'mail' ? 'md' : 'sm'}>
                {modalMode === 'add' && <AddBranch onSuccess={closeModal} />}
                {modalMode === 'edit' && selectedId && <EditBranch branchId={selectedId} onSuccess={closeModal} />}
                {modalMode === 'view' && selectedId && <ViewBranch branchId={selectedId} />}
            </SlidingModal>
        </AppLayout >
    )
}