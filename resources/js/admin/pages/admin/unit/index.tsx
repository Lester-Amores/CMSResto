...
import type { BreadcrumbItem, PaginatedProps, Unit, FlashMessages, ModalMode } from '@/admin/types';
...
interface UnitProps extends PaginatedProps {
    units: Unit[];
}

export default function UnitPage({ units, current_page, total_pages, total_rows, per_page }: UnitProps) {
    ...
    const handleMultiDelete = () => {
        ...
        showConfirmationPopup({
            ...
            cancelButton,
            onSuccess: () => {
                router.post(route('units.multi-delete'), { ids: selectedIdsRef.current }, {
                    headers: {
    ...
    const handleMultiRestore = () => {
        ...
        showConfirmationPopup({
            ...
            cancelButton,
            onSuccess: () => {
                router.post(route('units.multi-restore'), { ids: selectedIdsRef.current }, {
                    headers: {
                        'Content-Type': 'application/json',
                    },
                    preserveScroll: true,
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <SlidingModal isOpen={isModalOpen} onClose={closeModal} size={modalMode === 'mail' ? 'md' : 'sm'}>
                {modalMode === 'add' && <AddUnit onSuccess={closeModal} />}
                {modalMode === 'edit' && selectedId && <EditUnit unitId={selectedId} onSuccess={closeModal} />}
                {modalMode === 'view' && selectedId && <ViewUnit unitId={selectedId} />}
            </SlidingModal>
        </AppLayout >
    )
}