...
import type { BreadcrumbItem, PaginatedProps, Menu, FlashMessages, ModalMode } from '@/admin/types';
...
interface MenuProps extends PaginatedProps {
    menus: Menu[];
}

export default function MenuPage({ menus, current_page, total_pages, total_rows, per_page }: MenuProps) {
    ...
    const handleRestore = (id: number) => {
        ...
        showConfirmationPopup({
            ...
            cancelButton,
            onSuccess: () => {
                router.post(route('menus.restore'), { id }, {
                    preserveScroll: true,
                    onSuccess: (page) => {
                        const flash = page.props.flash as FlashMessages;
                        handleFlashMessages(flash)
    ...
    const handleMultiDelete = () => {
        ...
        showConfirmationPopup({
            ...
            cancelButton,
            onSuccess: () => {
                router.post(route('menus.multi-delete'), { ids: selectedIdsRef.current }, {
                    headers: {
    ...
    const handleMultiRestore = () => {
        ...
        showConfirmationPopup({
            ...
            cancelButton,
            onSuccess: () => {
                router.post(route('menus.multi-restore'), { ids: selectedIdsRef.current }, {
                    headers: {
                        'Content-Type': 'application/json',
                    },
                    preserveScroll: true,
    ...
    const data = menus?.map(menu => {
        const isDeleted: boolean = !!menu.deleted_at;
        return {
            id: menu.id,
            name: menu?.name,
            actions: (
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <SlidingModal isOpen={isModalOpen} onClose={closeModal} size={modalMode === 'mail' ? 'md' : 'sm'}>
                {modalMode === 'add' && <AddMenu onSuccess={closeModal} />}
                {modalMode === 'edit' && selectedId && <EditMenu menuId={selectedId} onSuccess={closeModal} />}
                {modalMode === 'view' && selectedId && <ViewMenu menuId={selectedId} />}
            </SlidingModal>
        </AppLayout >
    )
}